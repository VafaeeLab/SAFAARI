...
from utils_incremental import batch_generator, plot_embedding, set_random_seed
...
def visualized(model, data, show=True):
    ...
    result_name = "SAFAARI_Integration_Results/{}/{}/".format(data_set, data_name)
    if not os.path.exists(result_name):
        os.makedirs(result_name)

    if open_set:
        output_name_s = "SAFAARI_Integration_Results/{}/{}/{}_{}_to_{}_emb_op_source.csv".format(data_set, data_name, data_name, source_name, target_name)
    else:
        output_name_s = "SAFAARI_Integration_Results/{}/{}/{}_{}_to_{}_emb_source.csv".format(data_set, data_name, data_name, source_name, target_name)
    df_s = pd.DataFrame(source_feats[0])
    df_s.to_csv(output_name_s)

    if open_set:
        output_name_t = "SAFAARI_Integration_Results/{}/{}/{}_{}_to_{}_emb_op_target.csv".format(data_set, data_name, data_name,
                                                                                         source_name, target_name)
    else:
        output_name_t = "SAFAARI_Integration_Results/{}/{}/{}_{}_to_{}_emb_target.csv".format(data_set, data_name, data_name,
                                                                                      source_name, target_name)
...
def train(args, data, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if np.isnan(loss.item()):
            print("-"*100, "Error!")
        print("integration loss:", loss.item())

    visualized(model, ori_data)
...
def load_data_muris(path, data_name, domain_name, use_unknown):
    ...
    df = pd.read_csv('{}/opensetV2_Muris/ACC_Intersection_V2/{}/{}_{}_CmnGenes.csv'.format(path, data_name, data_name, domain_name))
    features = df[df.columns[1:]].to_numpy().astype(np.float32)
    raw_labels = df["cell types"].tolist()
