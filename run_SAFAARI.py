...
def visualized(model, data, show=False):
...
def train(args, src_data, tgt_data, tgt_test_data, device, data_name):
...
def test(model, tgt_test_data, num_classes, device, print_results=False):
    model.eval()
    ...
    if open_set:
        df.to_csv("data/FACS10X/{}/{}_to_{}_labels_op.csv".format(data_name, source_name, target_name))
    else:
        df.to_csv("data/FACS10X/{}/{}_to_{}_labels.csv".format(data_name, source_name, target_name))
...
if __name__ == '__main__':
    ...
    for data_name in ["bladder", "heart", "kidney", "mammary", "marrow", "muscle", "spleen"]:
        ...
        for target_name in target_domain_names:
            ...
            train(args, source_data, target_data, target_test_data, device, data_name=data_name)
